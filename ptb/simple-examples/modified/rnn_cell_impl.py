...
class MaskBasicLSTMCell(RNNCell):
  """Basic LSTM recurrent network cell.
  ...
  def __init__(self, num_units, forget_bias=1.0,
               state_is_tuple=True, activation=None, reuse=None, mask = None):
    """Initialize the basic LSTM cell.
    ...
    Args:
      ...
      CudnnCompatibleLSTMCell instead.
    """
    super(MaskBasicLSTMCell, self).__init__(_reuse=reuse)
    if not state_is_tuple:
      logging.warn("%s: Using a concatenated state is slower and will soon be "
                   "deprecated.  Use state_is_tuple=True.", self)
    self._num_units = num_units
...
class _Linear(object):
  ...
  def __call__(self, args):
    ...
    mask = np.ones(np.shape(self._weights))
    [m,n] = np.shape(mask)

    for i in range(m):
      for j in range(n):
        if(abs(i-(j%1000))>m/10):
          mask[i,j] = 0
    mask = tf.constant(mask,tf.float32)
    self._weights = tf.multiply(mask, self._weights)
    if len(args) == 1:
